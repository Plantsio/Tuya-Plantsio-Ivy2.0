...
int JPEGDEC::openRAM(uint8_t *pData, int iDataSize, xQueueHandle drawQueue) {
    memset((void*)&_jpeg, 0, sizeof(JPEGIMAGE));
//    _jpeg = {};
    _jpeg.ucMemType = JPEG_MEM_RAM;
    _jpeg.pfnRead = readRAM;
    _jpeg.pfnSeek = seekMem;
    _jpeg.pfnDraw = nullptr;
    _jpeg.use_draw_queue = true;
    _jpeg.draw_queue = drawQueue;
    _jpeg.pfnOpen = NULL;
    _jpeg.pfnClose = NULL;
    _jpeg.JPEGFile.iSize = iDataSize;
    _jpeg.JPEGFile.pData = pData;
...
//
int JPEGDEC::open(const char *szFilename, JPEG_OPEN_CALLBACK *pfnOpen, JPEG_CLOSE_CALLBACK *pfnClose,
...
//
int JPEGDEC::open(void *fHandle, int iDataSize, JPEG_CLOSE_CALLBACK *pfnClose, JPEG_READ_CALLBACK *pfnRead,
