...
#define MAX_BUFFERED_PIXELS 2560

/* added */
#define MAX_MCU                     5      /* how many MCUs can the decoder to output to a draw function */
#define MCU_SIZE                    256     /* this is set with the encoder using ffmpeg */
...
/* added struct to support queue draw */
typedef struct jpeg_draw_queue {
    JPEGDRAW *pool;
    xQueueHandle queue;
    uint8_t size;
    uint32_t *draw_queue_cnt;
} jpeg_draw_queue_t;
...
public:
    ...
    int openRAM(uint8_t *pData, int iDataSize, xQueueHandle drawQueue);
