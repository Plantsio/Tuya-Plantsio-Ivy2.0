...
//
static const uint8_t ucRangeTable[] = {0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8a, 0x8b, 0x8c,
                                       0x8d, 0x8e, 0x8f,
        ...
        if (pJPEG->ucHuffTableUsed & (1 << iTable)) {
            ...
            for (iBitNum = 1; iBitNum <= 16; iBitNum++) {
                ...
                while (iLen) {
                    ...
                    // does precalculating the DC value save time on ARM?
#ifndef USE_ARM_ASM
                    if (ucCode != 0 && (ucCode + iBitNum) <= 6 && pJPEG->ucMode !=
...
//                pFast = &pJPEG->ucAltHuff[0];
        while (pZig < pEnd) {
            ...
            {
                ulCode = ulBits << ulBitOff;
                ulTemp = ~(uint32_t) (((int32_t) ulCode)
                        >> (REGISTER_WIDTH - 1)); // slide sign bit across other 63 bits
    ...
    for (y = 0; y < cy && bContinue && iErr == 0; y++, jd.y += mcuCY) {
        ...
        for (x = 0; x < cx && bContinue && iErr == 0; x++) {
            ...
            if (pJPEG->ucMaxACCol == 0 || bThumbnail) // no AC components, save some time
            {
                ...
                JPEGIDCT(pJPEG, iLum0, pJPEG->JPCI[0].quant_tbl_no,
                         (pJPEG->ucMaxACCol | (pJPEG->ucMaxACRow << 8))); // first quantization table
            }
            ...
            if (pJPEG->ucSubSample > 0x11) // subsampling
            {
                ...
                if (pJPEG->ucMaxACCol == 0 || bThumbnail) // no AC components, save some time
                {
                    ...
                    JPEGIDCT(pJPEG, iLum1, pJPEG->JPCI[0].quant_tbl_no,
                             (pJPEG->ucMaxACCol | (pJPEG->ucMaxACRow << 8))); // first quantization table
                }
                if (pJPEG->ucSubSample == 0x22) {
                    ...
                    if (pJPEG->ucMaxACCol == 0 || bThumbnail) // no AC components, save some time
                    {
                        ...
                        JPEGIDCT(pJPEG, iLum2, pJPEG->JPCI[0].quant_tbl_no,
                                 (pJPEG->ucMaxACCol | (pJPEG->ucMaxACRow << 8))); // first quantization table
                    }
                    ...
                    if (pJPEG->ucMaxACCol == 0 || bThumbnail) // no AC components, save some time
                    {
                        ...
                        JPEGIDCT(pJPEG, iLum3, pJPEG->JPCI[0].quant_tbl_no,
                                 (pJPEG->ucMaxACCol | (pJPEG->ucMaxACRow << 8))); // first quantization table
                    }
                } // if 2:2 subsampling
            } // if subsampling used
            if (pJPEG->ucSubSample && pJPEG->ucNumComponents == 3) // if color (not CMYK)
            {
                ...
                if (pJPEG->ucMaxACCol == 0 || bThumbnail) // no AC components, save some time
                {
                    ...
                    JPEGIDCT(pJPEG, iCr, pJPEG->JPCI[1].quant_tbl_no,
                             (pJPEG->ucMaxACCol | (pJPEG->ucMaxACRow << 8))); // second quantization table
                }
                // second chroma
            ...
            if (xoff == iPitch || x == cx - 1) // time to draw
            {
                ...
                if ((jd.y - pJPEG->iYOffset + mcuCY) >
                    (pJPEG->iHeight >> iScaleShift)) { // last row needs to be trimmed
                    jd.iHeight = (pJPEG->iHeight >> iScaleShift) - (jd.y - pJPEG->iYOffset);
                }
                if (pJPEG->use_draw_queue) {
                    auto *draw = (JPEGDRAW *) heap_caps_malloc(sizeof(JPEGDRAW), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
                    draw->pPixels = (uint16_t *) heap_caps_malloc(MAX_MCU * MCU_SIZE * sizeof(uint16_t),
                                                                  MALLOC_CAP_DMA); /* todo use macro */
                    if (!draw->pPixels) {
                        log_w("failed to allocate buffer for mjpeg!!");
                    }else {
                        draw->x = jd.x;
                        draw->y = jd.y;
                        draw->iWidth = jd.iWidth;
                        draw->iHeight = jd.iHeight;
                        int len = jd.iWidth * jd.iHeight * 2;
                        memcpy(draw->pPixels, jd.pPixels, len);

                        xQueueSend(pJPEG->draw_queue, &draw, portMAX_DELAY);
                    }
                    bContinue = 1;

                } else {
                    bContinue = (*pJPEG->pfnDraw)(&jd); /* TT important */
                }
                jd.x += iPitch;
            ...
            if (pJPEG->iResInterval) {
                if (--pJPEG->iResCount == 0) {
                    ...
                    {
                        pJPEG->bb.ulBitOff += (8 - (pJPEG->bb.ulBitOff &
