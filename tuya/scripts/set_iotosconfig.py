...
def set_iotosconfig(make_yaml, osconfig, platform):
    ...
    kernel_context = ""
    kernel_deps = dependencies.get("adapter", [])
    for block in kernel_deps:
        if "Component" in block.keys():
            d = block['Component']
            if ('name' not in d.keys()) or ('locater' not in d.keys()):
                continue
            ...
            config_path = os.path.join(locater, name, 'IoTOSconfig')
            if os.path.exists(config_path):
                kernel_context += ('    rsource '+'../'+config_path+'\n')
    if kernel_context:
        kernel_context = KERNEL_CONTEXT_PREFIX + kernel_context + SUFFIX

    vendor_context = ""
    vendor_deps = dependencies.get("vendor", [])
    if platform != None:
        for block in vendor_deps:
    ...
    context += sdk_context + kernel_context + vendor_context
