...
## 28. module `SVC_NETMGR`

   No. | Name  | Value | message
-------|-------|-------|--------
1|`OPRT_SVC_NETMGR_NEED_FACTORY_RESET`|`(-0x2000)`|`-8192, 网络初始化配置校验失败，需要恢复出厂设置`

## 29. module `GW_BT_DEV`

   No. | Name  | Value | message
-------|-------|-------|--------
1|`OPRT_GW_BT_DEV_BLE_CONNECT_SUCCESS`|`(-0x3100)`|`-12544, ble设备连接成功`
2|`OPRT_GW_BT_DEV_BLE_CONNECT_CNT_EXCEED`|`(-0x3101)`|`-12545, ble设备连接数量已超出最大限制`
3|`OPRT_GW_BT_DEV_BLE_CONNECT_HAL_FAILED`|`(-0x3102)`|`-12546, ble设备连接HAL层接口返回失败`
4|`OPRT_GW_BT_DEV_BLE_CONNECT_HANDS_FAILED`|`(-0x3103)`|`-12547, ble设备连接握手失败`
5|`OPRT_GW_BT_DEV_BLE_CONNECT_HANDS_TIMEOUT`|`(-0x3104)`|`-12548, ble设备连接握手超时,请检查下设备是否正常上电状态,是否距离过远,是否被手机蓝牙连接`
6|`OPRT_GW_BT_DEV_BLE_CONNECT_GET_INFO_FAILED`|`(-0x3105)`|`-12549, ble设备连接get_info失败`
7|`OPRT_GW_BT_DEV_BLE_CONNECT_GET_LOGIN_KEY_FAILED`|`(-0x3106)`|`-12550, ble设备连接get_login_key失败`
8|`OPRT_GW_BT_DEV_BLE_CONNECT_PAIR_DATA_LEN_ERR`|`(-0x3107)`|`-12551, ble设备连接pair_data_len_err失败`
9|`OPRT_GW_BT_DEV_BLE_CONNECT_SESSION_OPEN_TIMEOUT`|`(-0x3108)`|`-12552, ble设备连接session_open超时`
10|`OPRT_GW_BT_DEV_BLE_CONNECT_SEND_FAILED`|`(-0x3109)`|`-12553, ble设备连接数据发送失败`
11|`OPRT_GW_BT_DEV_BLE_LOCAL_RESET`|`(-0x310a)`|`-12554, ble设备本地重置,请确认是否设备被手动重置`
12|`OPRT_GW_BT_DEV_BLE_SCAN_TIMEOUT`|`(-0x310b)`|`-12555, ble设备广播包扫描超时,请检查下设备是否正常上电状态,是否距离过远,是否被手机蓝牙连接`
13|`OPRT_GW_BT_DEV_BLE_DISCONNECT_SUCCESS`|`(-0x310c)`|`-12556, ble设备断开成功`
14|`OPRT_GW_BT_DEV_BEACON_DEV_NOT_FOUND`|`(-0x310d)`|`-12557, 设备没有发现`
15|`OPRT_GW_BT_DEV_BEACON_DEV_KEY1_NOT_FOUND`|`(-0x310e)`|`-12558, 设备KEY1没有发现，通信数据加密的key`
16|`OPRT_GW_BT_DEV_BEACON_DEV_NOT_BIND`|`(-0x310f)`|`-12559, 设备未绑定,请重新绑定设备`
17|`OPRT_GW_BT_DEV_BEACON_DECODE_PAIR_MSG_FAILED`|`(-0x3110)`|`-12560, 设备配网时,网关请求云端解密配网信息失败`
18|`OPRT_GW_BT_DEV_BEACON_KEY_RESP_TIMEOUT`|`(-0x3111)`|`-12561, 设备配网时,设备beaconkey应答超时`
19|`OPRT_GW_BT_DEV_BEACON_JOIN_NOTIFY_RESP_TIMEOUT`|`(-0x3112)`|`-12562, 设备配网时,设备入网通知应答超时`

## 30. module `GW_ZG_DEV`

   No. | Name  | Value | message
-------|-------|-------|--------
1|`OPRT_GW_ZG_DEV_ZB_HOST_TABLE_NULL`|`(-0x3200)`|`-12800, 设备未存在host表,将被移除,网关没有存储过该设备的信息`
2|`OPRT_GW_ZG_DEV_ZB_SEQ_INVALID`|`(-0x3201)`|`-12801, ZCL seq无效,当前数据包的seq小于上一个数据包的seq`
3|`OPRT_GW_ZG_DEV_ZB_HOST_TO_GATEWAY_ERR`|`(-0x3202)`|`-12802, 从host端发送数据到gateway组件出错,本地udp数据传输出错,建议重启网关`
4|`OPRT_GW_ZG_DEV_ZB_DATA_TOO_BIG`|`(-0x3203)`|`-12803, zigbee的数据长度太大被丢弃,建议检查数据格式`
5|`OPRT_GW_ZG_DEV_ZB_GW_TABLE_NULL`|`(-0x3204)`|`-12804, 网关设备表未初始化,建议重试配网`
6|`OPRT_GW_ZG_DEV_ZB_GW_RELIABLE_ERR`|`(-0x3205)`|`-12805, 可靠上报失败,建议检查数据格式`
7|`OPRT_GW_ZG_DEV_ZB_GW_TRD_DEV_ERR`|`(-0x3206)`|`-12806, 三级设备上报数据失败,建议检查数据格式`
8|`OPRT_GW_ZG_DEV_ZB_GATEWAY_DEV_ID_ERR`|`(-0x3207)`|`-12807, 数据下发设备id不存在,建议重新配网`
9|`OPRT_GW_ZG_DEV_ZB_GATEWAY_UPDATE_SCE_LIST_ERR`|`(-0x3208)`|`-12808, 数据下发场景列表不存在,可能没有验证该dp,建议重新创建场景`
10|`OPRT_GW_ZG_DEV_ZB_GATEWAY_MALLOC_ERR`|`(-0x3209)`|`-12809, GATEWAY层申请内存失败,内存不足,建议重启网关`
11|`OPRT_GW_ZG_DEV_ZB_GATEWAY_DATA_TYPE_ERR`|`(-0x320a)`|`-12810, 数据下发DP数据类型错误,检查下发数据格式`
12|`OPRT_GW_ZG_DEV_ZB_GATEWAY_TRD_DEV_DW_ERR`|`(-0x320b)`|`-12811, 三级设备数据下发失败,检查下发数据格式`
13|`OPRT_GW_ZG_DEV_ZB_GATEWAY_COM_DATALEN_ERR`|`(-0x320c)`|`-12812, 通用对接数据长度错误,检查下发数据格式`
14|`OPRT_GW_ZG_DEV_ZB_GATEWAY_SEND_DATA_FAILED`|`(-0x320d)`|`-12813, GATEWAY层向HOST层发送数据失败,检查下发数据格式`
15|`OPRT_GW_ZG_DEV_ZB_GATEWAY_NCRYPT_DATA_FAILED`|`(-0x320e)`|`-12814, 数据加密失败`
16|`OPRT_GW_ZG_DEV_ZB_GATWAY_STDRELIABLE_DW_FAILED`|`(-0x320f)`|`-12815, 标准透传命令发送失败,检查下发数据格式`
17|`OPRT_GW_ZG_DEV_ZB_GATWAY_CMD_TYPE_NOT_SUPPORT`|`(-0x3210)`|`-12816, 下发命令类型不支持,检查下发数据格式`
18|`OPRT_GW_ZG_DEV_ZB_HOST_RCV_SLABSCMD_MSG_LEN_ERR`|`(-0x3211)`|`-12817, 下发数据包长度错误,检查下发数据格式`
19|`OPRT_GW_ZG_DEV_ZB_HSOT_DEV_REJOIN`|`(-0x3212)`|`-12818, 下发数据时,设备处于rejoin过程中,建议等会重试`
20|`OPRT_GW_ZG_DEV_ZB_HOST_CMD_TYPE_NOT_SUPPORT`|`(-0x3213)`|`-12819, host命令类型不支持,检查下发数据格式`
21|`OPRT_GW_ZG_DEV_ZB_HOST_SEND_DATA_FAILED`|`(-0x3214)`|`-12820, host底层发送数据失败,建议重试`
22|`OPRT_GW_ZG_DEV_ZB_GATEWAY_DEV_BIND_ERR`|`(-0x3215)`|`-12821, 绑定出错,建议重新配网`
23|`OPRT_GW_ZG_DEV_ZB_GATEWAY_SET_HB_STAT_FAILED`|`(-0x3216)`|`-12822, GATEWAY层设置子设备心跳状态失败,该设备可能未适配`
24|`OPRT_GW_ZG_DEV_ZB_GATEWAY_JOIN_STAT_FALSE`|`(-0x3217)`|`-12823, 网关未开启配网,确保网关开启配网状态`
25|`OPRT_GW_ZG_DEV_ZB_GATEWAY_STAND_JOIN_ERR`|`(-0x3218)`|`-12824, 标准入网失败,该设备可能未适配`
26|`OPRT_GW_ZG_DEV_ZB_GATEWAY_DEV_NOT_SUPPORT`|`(-0x3219)`|`-12825, 子设备不支持,该设备可能未适配`
27|`OPRT_GW_ZG_DEV_ZB_HOST_RCV_JOIN_MSG_ERR`|`(-0x321a)`|`-12826, zigbeehost层接收MSG长度异常,检查下发数据格式`
28|`OPRT_GW_ZG_DEV_ZB_GATEWAY_TRD_DEVID_ERR`|`(-0x321b)`|`-12827, 三级设备的设备ID错误,建议重新配网`
29|`OPRT_GW_ZG_DEV_ZB_GATEWAY_THR_DEV_TPE_ERR`|`(-0x321c)`|`-12828, 三级设备的设备类型错误,建议重新配网`
30|`OPRT_GW_ZG_DEV_ZB_GATEWAY_RS_PROFILE_NOT_SUPPORT`|`(-0x321d)`|`-12829, 接收ZIGBEE PROFILE不支持,该设备可能未适配`
31|`OPRT_GW_ZG_DEV_ZB_SUBDEV_LEAVE_SELF`|`(-0x321e)`|`-12830, 子设备主动离网,可能设备被重置`

## 31. module `GW_BASE_LINKAGE`

   No. | Name  | Value | message
-------|-------|-------|--------
1|`OPRT_GW_BASE_LINKAGE_INIT_FAIL`|`(-0x3300)`|`-13056, 联动模块初始化失败`
2|`OPRT_GW_BASE_LINKAGE_COND_CHECK_FAIL`|`(-0x3301)`|`-13057, 联动条件不满足`
3|`OPRT_GW_BASE_LINKAGE_RULE_ID_NOT_FIND`|`(-0x3302)`|`-13058, 联动规则ID不存在,请检查规则是否存在`
4|`OPRT_GW_BASE_LINKAGE_RULE_ID_DISABLE`|`(-0x3303)`|`-13059, 联动规则未启用,请检查规则是否启用`
5|`OPRT_GW_BASE_LINKAGE_RULE_IS_UPDATED`|`(-0x3304)`|`-13060, 联动规则在更新中,请稍后重试`
6|`OPRT_GW_BASE_LINKAGE_FLOW_CNTL`|`(-0x3305)`|`-13061, 联动流控,可能触发过于频繁,请稍后再重试,也可能存在死循环,请检查联动是否死循环`
7|`OPRT_GW_BASE_LINKAGE_THREAD_CREATE_FAIL`|`(-0x3306)`|`-13062, 联动线程创建失败`
8|`OPRT_GW_BASE_LINKAGE_COND_CHECK_NO_ID_MATCH`|`(-0x3307)`|`-13063, 联动条件不满足(设备ID无匹配项)`
9|`OPRT_GW_BASE_LINKAGE_COND_CHECK_NO_DP_MATCH`|`(-0x3308)`|`-13064, 联动条件不满足(条件DP无匹配项)`

## 32. module `GW_ZG_SUB`

   No. | Name  | Value | message
-------|-------|-------|--------
1|`OPRT_GW_ZG_SUB_ERR0`|`(-0x3400)`|`-13312, 错误0`
2|`OPRT_GW_ZG_SUB_ERR1`|`(-0x3401)`|`-13313, 错误1`
3|`OPRT_GW_ZG_SUB_ERR2`|`(-0x3402)`|`-13314, 错误2:FIB重启`
4|`OPRT_GW_ZG_SUB_ERR3`|`(-0x3403)`|`-13315, 错误3:升级重启`
5|`OPRT_GW_ZG_SUB_ERR4`|`(-0x3404)`|`-13316, 错误4:外部IO异常重启`
6|`OPRT_GW_ZG_SUB_ERR5`|`(-0x3405)`|`-13317, 错误5:低电压重启,可能是断电重启,也可能是供电问题`
7|`OPRT_GW_ZG_SUB_ERR6`|`(-0x3406)`|`-13318, 错误6:看门狗重启`
8|`OPRT_GW_ZG_SUB_ERR7`|`(-0x3407)`|`-13319, 错误7:软件主动重启`
9|`OPRT_GW_ZG_SUB_ERR8`|`(-0x3408)`|`-13320, 错误8:软件crash重启`
10|`OPRT_GW_ZG_SUB_ERR9`|`(-0x3409)`|`-13321, 错误9:Flash异常重启`
11|`OPRT_GW_ZG_SUB_ERR10`|`(-0x340a)`|`-13322, 错误10:不可恢复硬件异常重启`
12|`OPRT_GW_ZG_SUB_ERR11`|`(-0x340b)`|`-13323, 错误11:硬件内部CPU访问错误`
13|`OPRT_GW_ZG_SUB_ERR12`|`(-0x340c)`|`-13324, 错误12:掉电重启,可能是断电重启,也可能是供电问题`
14|`OPRT_GW_ZG_SUB_ERR13`|`(-0x340d)`|`-13325, 错误13`
15|`OPRT_GW_ZG_SUB_ERR14`|`(-0x340e)`|`-13326, 错误14`
16|`OPRT_GW_ZG_SUB_ERR15`|`(-0x340f)`|`-13327, 错误15`
17|`OPRT_GW_ZG_SUB_ERR16`|`(-0x3410)`|`-13328, 错误16:重启`
18|`OPRT_GW_ZG_SUB_ERR17`|`(-0x3411)`|`-13329, 错误17:失联`
19|`OPRT_GW_ZG_SUB_ERR18`|`(-0x3412)`|`-13330, 错误18:主动离网,请确认客户是否本地触发退网`
20|`OPRT_GW_ZG_SUB_ERR19`|`(-0x3413)`|`-13331, 错误19:被动离网,请确认是不是被APP或者遥控器移除`
21|`OPRT_GW_ZG_SUB_ERR20`|`(-0x3414)`|`-13332, 错误20:IO异常`
22|`OPRT_GW_ZG_SUB_ERR21`|`(-0x3415)`|`-13333, 错误21:内存泄漏`
23|`OPRT_GW_ZG_SUB_ERR22`|`(-0x3416)`|`-13334, 错误22:短地址改变`
24|`OPRT_GW_ZG_SUB_ERR23`|`(-0x3417)`|`-13335, 错误23:收到恢复出厂设置`
25|`OPRT_GW_ZG_SUB_ERR24`|`(-0x3418)`|`-13336, 错误24:收到F0命令`
26|`OPRT_GW_ZG_SUB_ERR25`|`(-0x3419)`|`-13337, 错误25:唤醒时间过长`
27|`OPRT_GW_ZG_SUB_ERR26`|`(-0x341a)`|`-13338, 错误26:收到异常恢复出厂设置`
...
## 43. module `OS_ADAPTER_I2C`

   No. | Name  | Value | message
-------|-------|-------|--------
1|`OPRT_OS_ADAPTER_I2C_OPEN_FAILED`|`(-0x6f00)`|`-28416, I2C 打开失败`
2|`OPRT_OS_ADAPTER_I2C_CLOSE_FAILED`|`(-0x6f01)`|`-28417, I2C 关闭失败`
3|`OPRT_OS_ADAPTER_I2C_READ_FAILED`|`(-0x6f02)`|`-28418, I2C 读取失败`
4|`OPRT_OS_ADAPTER_I2C_WRITE_FAILED`|`(-0x6f03)`|`-28419, I2C 写入失败`
5|`OPRT_OS_ADAPTER_I2C_INVALID_PARM`|`(-0x6f04)`|`-28420, 参数无效`
6|`OPRT_OS_ADAPTER_I2C_INIT_FAILED`|`(-0x6f05)`|`-28421, 初始化失败`
7|`OPRT_OS_ADAPTER_I2C_DEINIT_FAILED`|`(-0x6f06)`|`-28422, 释放失败`
8|`OPRT_OS_ADAPTER_I2C_MODE_ERR`|`(-0x6f07)`|`-28423, 模式错误`
9|`OPRT_OS_ADAPTER_I2C_BUSY`|`(-0x6f08)`|`-28424, 忙`
10|`OPRT_OS_ADAPTER_I2C_TIMEOUT`|`(-0x6f09)`|`-28425, 超时`
11|`OPRT_OS_ADAPTER_I2C_MASTER_LOST_ARB`|`(-0x6f0a)`|`-28426, 主机仲裁失败`
12|`OPRT_OS_ADAPTER_I2C_BUS_ERR`|`(-0x6f0b)`|`-28427, 总线错误`
13|`OPRT_OS_ADAPTER_I2C_TRANS_ERR`|`(-0x6f0c)`|`-28428, 传输错误`
14|`OPRT_OS_ADAPTER_I2C_ADDR_NO_ACK `|`(-0x6f0d)`|`-28429, 无ACK`
15|`OPRT_OS_ADAPTER_I2C_IRQ_INIT_FAILED `|`(-0x6f0e)`|`-28430, 中断初始化失败`
16|`OPRT_OS_ADAPTER_I2C_RESERVE1_ERR`|`(-0x6f0f)`|`-28431, RESERVE ERR1`
17|`OPRT_OS_ADAPTER_I2C_RESERVE2_ERR`|`(-0x6f10)`|`-28432, RESERVE ERR2`
18|`OPRT_OS_ADAPTER_I2C_RESERVE3_ERR`|`(-0x6f11)`|`-28433, RESERVE ERR3`

## 44. module `OS_ADAPTER_BLE`

   No. | Name  | Value | message
-------|-------|-------|--------
1|`OPRT_OS_ADAPTER_BLE_HANDLE_ERROR`|`(-0x7000)`|`-28672, Ble 句柄错误`
2|`OPRT_OS_ADAPTER_BLE_BUSY`|`(-0x7001)`|`-28673, Ble 繁忙`
3|`OPRT_OS_ADAPTER_BLE_TIMEOUT`|`(-0x7002)`|`-28674, Ble 超时`
4|`OPRT_OS_ADAPTER_BLE_RESERVED1`|`(-0x7003)`|`-28675, Ble Reserved Error Code 1`
5|`OPRT_OS_ADAPTER_BLE_RESERVED2`|`(-0x7004)`|`-28676, Ble Reserved Error Code 2`
6|`OPRT_OS_ADAPTER_BLE_RESERVED3`|`(-0x7005)`|`-28677, Ble Reserved Error Code 3`
7|`OPRT_OS_ADAPTER_BLE_INIT_FAILED`|`(-0x7006)`|`-28678, Ble 初始化失败`
8|`OPRT_OS_ADAPTER_BLE_DEINIT_FAILED`|`(-0x7007)`|`-28679, Ble 释放失败`
9|`OPRT_OS_ADAPTER_BLE_GATT_CONN_FAILED`|`(-0x7008)`|`-28680, Ble GATT连接失败`
10|`OPRT_OS_ADAPTER_BLE_GATT_DISCONN_FAILED`|`(-0x7009)`|`-28681, Ble GATT断开失败`
11|`OPRT_OS_ADAPTER_BLE_ADV_START_FAILED`|`(-0x700a)`|`-28682, Ble 开启广播失败`
12|`OPRT_OS_ADAPTER_BLE_ADV_STOP_FAILED`|`(-0x700b)`|`-28683, Ble 停止广播失败`
13|`OPRT_OS_ADAPTER_BLE_SCAN_START_FAILED`|`(-0x700c)`|`-28684, Ble 开启扫描失败`
14|`OPRT_OS_ADAPTER_BLE_SCAN_STAOP_FAILED`|`(-0x700d)`|`-28685, Ble 停止扫描失败`
15|`OPRT_OS_ADAPTER_BLE_SVC_DISC_FAILED`|`(-0x700e)`|`-28686, Ble 服务发现失败`
16|`OPRT_OS_ADAPTER_BLE_CHAR_DISC_FAILED`|`(-0x700f)`|`-28687, Ble 特征值发现失败`
17|`OPRT_OS_ADAPTER_BLE_DESC_DISC_FAILED`|`(-0x7010)`|`-28688, Ble 特征值描述符发现失败`
18|`OPRT_OS_ADAPTER_BLE_NOTIFY_FAILED`|`(-0x7011)`|`-28689, Ble Peripheral Notify失败`
19|`OPRT_OS_ADAPTER_BLE_INDICATE_FAILED`|`(-0x7012)`|`-28690, Ble Peripheral Indicate失败`
20|`OPRT_OS_ADAPTER_BLE_WRITE_FAILED`|`(-0x7013)`|`-28691, Ble Central 写失败`
21|`OPRT_OS_ADAPTER_BLE_READ_FAILED`|`(-0x7014)`|`-28692, Ble Central 读失败`
22|`OPRT_OS_ADAPTER_BLE_MTU_REQ_FAILED`|`(-0x7015)`|`-28693, Ble Central MTU请求失败`
23|`OPRT_OS_ADAPTER_BLE_MTU_REPLY_FAILED`|`(-0x7016)`|`-28694, Ble Peripheral MTU 响应失败`
24|`OPRT_OS_ADAPTER_BLE_CONN_PARAM_UPDATE_FAILED`|`(-0x7017)`|`-28695, Ble 连接参数更新失败`
25|`OPRT_OS_ADAPTER_BLE_CONN_RSSI_GET_FAILED`|`(-0x7018)`|`-28696, Ble 连接信号强度获取失败`
26|`OPRT_OS_ADAPTER_BLE_MESH_INVALID_OPCODE`|`(-0x7019)`|`-28697, Ble Mesh 无效的opcode`
27|`OPRT_OS_ADAPTER_BLE_MESH_INVALID_ELEMENT`|`(-0x701a)`|`-28698, Ble Mesh 无效的element`
28|`OPRT_OS_ADAPTER_BLE_MESH_INVALID_MODEL`|`(-0x701b)`|`-28699, Ble Mesh 无效的model`
29|`OPRT_OS_ADAPTER_BLE_MESH_INVALID_ADDR`|`(-0x701c)`|`-28700, Ble Mesh 无效的source, virtual或destination address`
